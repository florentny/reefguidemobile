...
// Minimal valid 1×1 white JPEG used as a stand-in for any image asset.
...
void setupMockAssets() {
  ...
  TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
    ...
    // Return an empty StandardMessageCodec-encoded map so that Image.asset
    // can resolve the manifest without crashing (type=13, size=0).
    if (key == 'AssetManifest.bin') {
      return ByteData.view(Uint8List.fromList([13, 0]).buffer);
    }
    if (key.endsWith('.jpg') || key.endsWith('.jpeg') || key.endsWith('.png')) {
      return ByteData.view(Uint8List.fromList(_minimalJpeg).buffer);
    }
    return null;
  });
}
